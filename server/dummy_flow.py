        ...
        while True:
            ...
            if not client_message or client_message.lower() == "exit":
                ...
                if filtered_string[1] in ('b', 'B'):
                    ...
                    server_response = "Transaction successful" if transaction_result else "Transaction failed"
...
def udp_server(udp_port, tcp_port, is_leader_flag):
    ...
    if is_leader_flag:
        print(f"Leader server is waiting for broadcast messages on UDP port {udp_port}...")
    else:
        print(f"Follower server is waiting for broadcast messages on UDP port {udp_port}...")
        ...
        try:
            ...
            if is_leader_flag:
                ...
                if message == "NEW_SERVER":
                    ...
                    unicast_message = f"ACK_LEADER {tcp_port} SERVER_GROUP {server_group}"
...
if _name_ == '_main_':
    ...
    udp_thread = threading.Thread(target=udp_server, args=(SERVER_UDP_PORT, SERVER_TCP_PORT, IS_LEADER))
    udp_thread.start()
